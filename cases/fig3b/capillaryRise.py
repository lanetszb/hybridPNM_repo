...
P_ini = 1.e2
PBcTypes = {"inlet": "Neumann", "outlet": "Dirichlet"}
valuePInlet = u_inlet * width * depth
valuePInletIni = 0.
valuePOutlet = P_ini
...
poresCoordinates = {0: [0., 5.e-3], 1: [0., 0.]}
throatsPores = {0: [0, 1]}
...
cells_file_name = 'capillaryRise.pvd'
...
for i in range(len(ts_out_coarse)):
    ...
    cells_files_names.append('capillaryRise_' + str(round(time, round_output_time)) + '.vtu')
...
X_cells = np.linspace(0, 5.e-3, netgrid._cellsN)
P_array = P_array_times[51][::-1]
S0_array = S0_array_times[51][::-1]

d = {'X_cells': X_cells, 'P_array': P_array, 'S0_array': S0_array}
df = pd.DataFrame(data=d)
df.to_csv('capillary_pnm_imb.csv')
